import {circle, computeBoundaries, css, isOver, line, toDate, toCoords, computeYRatio, computeXRatio} from "./utils";
...
const HEIGHT = 200
const DPI_KOEF = 2
const DPI_WIDTH = WIDTH * DPI_KOEF
const DPI_HEIGHT = HEIGHT * DPI_KOEF
const ROWS_COUNT = 5
const PADDING = 40
...
export function chart(root, data) {
  ...
  function mouseleave({clientX, clientY}) {
    proxy.mouse = null
    tip.hide()
  }
  ...
  function paint() {
    ...
    yData.map(toCoords(xRatio, yRatio, DPI_HEIGHT, PADDING, yMin)).forEach((coords, idx) => {
      ...
      line(ctx, coords, {color, translate: 0})
